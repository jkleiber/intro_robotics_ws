...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber bump_sub = obstacle_avoid_node.subscribe(obstacle_avoid_node.resolveName("/mobile_base/events/bumper"), 10, &collision_callback);

    //Publish state to the collision topic
    collision_pub = obstacle_avoid_node.advertise<reactive_robot::collision>(obstacle_avoid_node.resolveName("/reactive_robot/collision"), 10);
